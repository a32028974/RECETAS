...
export function configurarCalculoPrecios() {
  ...
  function actualizarTotalYSaldo() {
    ...
    if (saldoInput) saldoInput.value = saldo.toFixed(2);
  }

  [precioCristal, precioArmazon, precioOtro, senaInput]
    .filter(Boolean)
    .forEach(el => el.addEventListener("input", actualizarTotalYSaldo));
}
